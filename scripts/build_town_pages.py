...
PROJECTS = {
    ...
    "exhale": {
        ...
        "area": "Lakeshore &amp; Dixie",
        "hero": "exhale-hero",
        "hero_alt": "Exhale Residences on Lakeshore, aerial rendering of the terraces",
        ...
        "gallery_title": "Renderings &amp; <em>amenities</em>",
        "gallery": [
            ("exhale-entry", "The entrance on Lakeshore Road"),
            ("exhale-towns-hero", "Exhale on Lakeshore from the street"),
            ("exhale-lobby", "Lobby"),
            ("exhale-lobby-detail", "Lobby detail"),
            ("exhale-suite", "Suite kitchen and living"),
            ("exhale-ensuite", "Suite ensuite"),
            ("exhale-party-room", "Party room"),
            ("exhale-gym", "Gym"),
            ("exhale-kids", "Kids&rsquo; room"),
            ("exhale-rooftop", "Rooftop terrace"),
            ("exhale-rooftop-2", "Rooftop terrace from above"),
            ("exhale-outdoor", "Outdoor amenity terrace"),
            ("exhale-amenity-plan", "Amenity floor plan"),
...
PLANS_CSS = """
    /* floor plan book */
    .plans-sec { scroll-margin-top: 70px; }
...
(function () {
  ...
  window.caUnlockPlans = function () {
    var locked = sec.querySelectorAll('.plan:not(.is-open)');
    if (!locked.length) return;                       // nothing is gated on this page
    sec.classList.add('is-unlocked');
    Array.prototype.forEach.call(locked, open);
  ...
  // The chosen filter lives in the link (?beds=3#floor-plans), so a shared link opens the same view.
  function applyFilter(t, remember) {
    var chip = sec.querySelector('.plan-chip[data-filter="' + t + '"]');
  ...
  function show(fig) {
    ...
    setParam('plan', fig.id.replace(/^plan-/, ''));
  }
  function close() {
