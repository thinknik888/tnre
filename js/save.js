...
(function() {
  var saved = JSON.parse(localStorage.getItem('ca_saved_plans') || '[]');
  ...
  style.textContent = '\
    ...
    .save-modal .cancel{background:none;color:#8a8a84;margin-top:0.5rem;font-size:0.75rem;text-transform:none;letter-spacing:0;}\
  ...
  // Add save buttons to all floor plan cards
  function addSaveButtons() {
    // Grid cards (fp-card with fp-image)
  ...
  function addBtn(container, planId) {
    ...
    btn.className = 'save-btn' + (saved.indexOf(planId) > -1 ? ' saved' : '');
    btn.innerHTML = heartSvg;
    btn.addEventListener('click', function(e) {
      e.stopPropagation();
      if (!user) {
        showLoginModal(planId);
      } else {
        toggleSave(planId, btn);
      }
    });
    container.appendChild(btn);
  }

  function toggleSave(planId, btn) {
    var idx = saved.indexOf(planId);
    if (idx > -1) {
      saved.splice(idx, 1);
    ...
    localStorage.setItem('ca_saved_plans', JSON.stringify(saved));
    updateBadge();
  }

  // Login modal
  var modalEl = null;
  function showLoginModal(planId) {
    if (!modalEl) {
      modalEl = document.createElement('div');
      modalEl.className = 'save-modal-overlay';
      modalEl.innerHTML = '<div class="save-modal">\
        <h3>Save Floor Plans</h3>\
        <p>No account needed — just your name and number so we can keep your list ready.</p>\
        ...
        <button class="cancel" id="save-cancel">Maybe later</button>\
      </div>';
    ...
    document.getElementById('save-submit').onclick = function() {
      ...
      document.querySelectorAll('.save-btn').forEach(function(b) {
        ...
        var card = container.closest('.fp-card') || container.closest('.fp-row');
        if (card) {
          var oc = card.getAttribute('onclick') || '';
          if (oc.indexOf("'" + planId + "'") > -1) {
            toggleSave(planId, b);
          }
        }
      });
